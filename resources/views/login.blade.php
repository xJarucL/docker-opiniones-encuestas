...
<div class="min-h-screen flex flex-col items-center justify-center bg-purple-800 px-4 sm:px-6 lg:px-8 py-8 space-y-6">
    ...
    <div class="text-center px-2 sm:px-0">
        <h1 class="text-3xl sm:text-4xl md:text-5xl font-bold text-white">Encuestas y Opiniones</h1>
        <p class="text-purple-200 mt-2 text-sm sm:text-base md:text-lg">
            Inicia sesión para participar y compartir tus opiniones
        </p>
    </div>

    <div class="w-full max-w-sm sm:max-w-md md:max-w-lg bg-white rounded-2xl shadow-lg p-6 sm:p-8 animate-fade-slide hover:shadow-2xl transition-shadow duration-300">

        <x-msj-alert />

        <h2 class="text-xl sm:text-2xl font-bold text-gray-800 text-center mb-6">Iniciar sesión</h2>

        <form id="form-insertar" data-url="{{ route('iniciando') }}" class="space-y-4 sm:space-y-5">
            @csrf
            ...
            <div>
                <input type="submit" value="Ingresar"
                       class="w-full bg-purple-600 text-white font-semibold px-6 py-3 sm:py-3 rounded-lg shadow-md
                              hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500
                              transition duration-200 cursor-pointer text-base sm:text-lg" />
            </div>

            <div class="text-center mt-3">
                <a href="{{ route('recuperar_contraseña') }}" class="text-sm sm:text-base text-gray-600 hover:underline">
