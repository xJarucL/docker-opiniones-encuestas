...
<head>
    ...
    {{-- Usamos la variable $pregunta que viene del controlador --}}
    <title>Podio - {{ $pregunta->texto ?? 'Resultados' }}</title>
    
    <style>
        /* [ ... TODO TU CSS DE ANIMACIÓN ... ] */
        /* (Pego todo el CSS que me enviaste para asegurar que funcione) */
...
<body>
    ...
    <div id="section2">
        <div class="box">
            {{-- Usamos la variable $pregunta del controlador --}}
            <h1>Los nominados a<br>{{ $pregunta->texto ?? 'la pregunta' }}<br>son .....</h1>
        </div>
    </div>
        ...
        @if(isset($resultados) && count($resultados) > 0)
            ...
            @if(isset($resultados[2]))
                <div class="podium-pillar pillar-3">
                    {{-- CORREGIDO: Usar 'respuesta' en lugar de 'opcion' --}}
                    <div class="pillar-name">{{ $resultados[2]->respuesta }}</div> 
            ...
            @if(isset($resultados[0]))
                <div class="podium-pillar pillar-1">
                    {{-- CORREGIDO: Usar 'respuesta' en lugar de 'opcion' --}}
                    <div class="pillar-name">{{ $resultados[0]->respuesta }}</div>
            ...
            @if(isset($resultados[1]))
                <div class="podium-pillar pillar-2">
                    {{-- CORREGIDO: Usar 'respuesta' en lugar de 'opcion' --}}
                    <div class="pillar-name">{{ $resultados[1]->respuesta }}</div>
    ...
    {{-- 
      BOTÓN CORREGIDO: 
      Ahora usa las variables del controlador `PresentaciontwoController@podio`
      ($hayMasPreguntas, $encuestaId, $preguntaIndex)
    --}}
    @if($hayMasPreguntas)
        <a href="{{ route('podiotwo', ['encuestaId' => $encuestaId, 'preguntaIndex' => $preguntaIndex + 1]) }}" 
           id="verResultadosBtn" 
    ...
    <script>
        // Tu script de animación (sin cambios, ya funciona)
